...
class SqsInboundPropertiesTest {
  ...
  void usesCredentialAuthenticationAndRegionWhenBound() {
    var properties = new SqsInboundProperties();
    properties.setAwsCredential(CREDENTIAL);
  ...
  void fallsBackToInlineWhenNoCredential() {
    var properties = new SqsInboundProperties();
    properties.setAuthentication(new AwsStaticCredentialsAuthentication("inline-ak", "inline-sk"));
    properties.setConfiguration(new AwsBaseConfiguration("us-east-1", null));

    assertThat(((AwsStaticCredentialsAuthentication) properties.getAuthentication()).accessKey())
        .isEqualTo("inline-ak");
    assertThat(properties.getConfiguration().region()).isEqualTo("us-east-1");
  }
  ...
  void credentialOnlySatisfiesAuthenticationValidation() {
    ...
    assertThat(properties.isAuthenticationPresent()).isTrue();
  }
}
