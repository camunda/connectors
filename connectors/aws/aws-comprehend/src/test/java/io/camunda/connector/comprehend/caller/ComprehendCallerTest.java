...
class ComprehendCallerTest {
  ...
  @Test
  void prepareDocumentReaderConfigWithEmptyReadActionReturnNull() {
    ComprehendSyncRequestData request = prepareSyncData(true, true);
    DocumentReaderConfig documentReader = caller.prepareDocumentReaderConfig(request);
    assertThat(documentReader).isNull();
  }
  ...
  void
    ...
    DocumentReaderConfig readerConfig = caller.prepareDocumentReaderConfig(request);
