...
public class AwsLambdaResult {
  ...
  public AwsLambdaResult(final InvokeResponse invokeResponse, final ObjectMapper objectMapper) {
    this.statusCode = invokeResponse.statusCode();
    this.executedVersion = invokeResponse.executedVersion();
    try {
      this.payload = objectMapper.readValue(invokeResponse.payload().asByteArray(), Object.class);
    } catch (IOException e) {
