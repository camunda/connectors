...
- **Job supersession / `NOT_FOUND`**: a completing tool creates a new job; the stale in-flight job may be rejected.
  ...
  `jobLease(...)` on the update and create-history writes, so a stale activation's agent-instance writes are rejected
  (`NOT_FOUND`) too. Gateway/MCP/A2A connectors are not leased. `ai-agent.md` §10, §23.
