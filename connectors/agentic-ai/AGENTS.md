...
- **Job supersession / `NOT_FOUND`**: a completing tool creates a new job; the stale in-flight job may be rejected.
  ...
  superseded activation; `CamundaAgentInstanceClient` forwards the lease token via `jobLease(...)` on the update and
  create-history writes, so a stale activation's agent-instance writes are rejected (`NOT_FOUND`) too. `ai-agent.md` §10, §23.
