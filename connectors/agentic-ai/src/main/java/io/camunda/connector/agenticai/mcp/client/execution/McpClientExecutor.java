...
public class McpClientExecutor {
  ...
  private @NonNull McpClientResult executeRequest(McpClientDelegate clientDelegate, McpClientOperation operation, FilterOptions filterOptions) {
    return switch (operation.method()) {
      ...
      case LIST_RESOURCE_TEMPLATES -> clientDelegate.listResourceTemplates(filterOptions.resourceFilters());
      case READ_RESOURCE -> clientDelegate.readResource(operation.params(), filterOptions.resourceFilters());
      case LIST_PROMPTS -> clientDelegate.listPrompts(filterOptions.promptFilters());
      case GET_PROMPT -> clientDelegate.getPrompt(operation.params(), filterOptions.promptFilters());
    };
  }
}
