...
public class A2aClientWebhookExecutable implements WebhookConnectorExecutable {
  ...
  public A2aClientWebhookExecutable(
      ObjectMapper objectMapper, A2aSdkObjectConverter a2aSdkObjectConverter) {
  ...
  public void activate(InboundConnectorContext context) {
    ...
    verifyClientResponse(props.clientResponse());
  ...
  public WebhookResult triggerWebhook(WebhookProcessingPayload payload) {
    LOGGER.trace("Triggered A2A webhook with context {} and payload {}", props.context(), payload);
  ...
  private void verifyClientResponse(String clientResponse) {
    try {
      objectMapper.readValue(clientResponse, A2aTask.class);
    } catch (JsonProcessingException e) {
      throw new RuntimeException("Failed to convert the clientResponse variable to an A2A task", e);
    }
  }
