...
import io.camunda.connector.agenticai.a2a.client.model.result.A2AClientAgentCardResultAgentSkillBuilder;
...
class A2AClientHandlerImplTest {
  ...
  class FetchAgentCard {
    ...
    void fetchAgentCardWithCustomLocation() {
      ...
      try (MockedStatic<A2A> a2aStatic = mockStatic(A2A.class)) {
        ...
        assertThat(result).isInstanceOf(A2AClientAgentCardResult.class);
        assertAgentCard(
            (A2AClientAgentCardResult) result, DEFAULT_INPUT_MODES, DEFAULT_OUTPUT_MODES);
      }
    }
    ...
    void fetchAgentCardWithSkillsInputAndOutputMode() {
      ...
      try (MockedStatic<A2A> a2aStatic = mockStatic(A2A.class)) {
        ...
        a2aStatic.verify(() -> A2A.getAgentCard(AGENT_URL, null, Collections.emptyMap()));
        assertThat(result).isInstanceOf(A2AClientAgentCardResult.class);
        assertAgentCard((A2AClientAgentCardResult) result, inputModes, outputModes);
      }
    }
    ...
    private void assertAgentCard(
        A2AClientAgentCardResult agentCardResult,
      ...
      assertThat(agentCardResult.skills())
          .containsExactly(
              new A2AClientAgentCardResultAgentSkillBuilder()
                  .id("hotel-booking")
                  .name("Hotel Booking")
                  .description("Book a hotel room")
                  .tags(List.of("booking", "hotel"))
