...
/**
 * Migrates a persisted {@code agentContext} JSON tree to {@link
 * AgentContext#CURRENT_SCHEMA_VERSION} before it is bound, dispatching by schema version to a named
 * migration step per version gap. Also exposes {@link ToolCallResultUpcaster}, the raw-JSON upcast
 * helper for the one migration step introduced so far, reused by the Camunda document store payload
 * (self-describing with its own {@code schemaVersion}, gated the same way before calling in).
 *
 * <p>Operates on raw {@link JsonNode} trees so the transform runs before object binding.
 */
public final class AgentContextSchemaMigration {
  ...
  /**
   * Dispatches to the migration step for {@code fromVersion}, returning the resulting version. A
   * future v1-&gt;v2 step is a plain {@code migrateV1ToV2}; the first step stays named after
   * "unversioned" rather than "v0" because {@link AgentContext#UNVERSIONED_SCHEMA_VERSION} means no
   * {@code schemaVersion} field was present at all (pre-8.9 legacy state), not a deliberate schema
   * version.
   */
  ...
  private static int migrateUnversionedToV1(ObjectNode agentContextObject, ObjectMapper mapper) {
    ...
    if (conversation != null
      ...
      ToolCallResultUpcaster.upcastMessages(conversation.get(FIELD_MESSAGES), mapper);
    }
    return 1;
  }
  ...
  /**
   * Lifts the old flat tool-call-result {@code content} value into the current structured {@link
   * Content} list shape, shared by every persisted root that carries a {@code messages} array: the
   * in-process {@code agentContext} conversation and the Camunda document store payload.
   */
  public static final class ToolCallResultUpcaster {
    ...
    private ToolCallResultUpcaster() {}
     ...
     * Guards defensively against non-array/non-object shapes; a pointer-style conversation
     * (document) has no {@code messages} node and is left untouched by the caller.
     */
    ...
    /** Upcasts a persisted tool-call-results array in place. */
    private static void upcastToolCallResults(@Nullable JsonNode resultsNode, ObjectMapper mapper) {
     ...
     *   <li>anything else (object, array — including a legacy gateway {@code List<McpContent>}
     *       whose elements happen to share {@code text}/{@code object}/{@code document} type
     *       discriminators with domain {@link Content} — number, or boolean) -&gt; a single {@link
     *       ObjectContent} wrapping the whole node, opaque and unsplit
     * </ul>
     */
    private static List<Content> liftLegacyContent(@Nullable JsonNode node, ObjectMapper mapper) {
