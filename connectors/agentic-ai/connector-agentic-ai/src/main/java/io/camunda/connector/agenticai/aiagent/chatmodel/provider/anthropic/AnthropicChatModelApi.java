...
public class AnthropicChatModelApi implements ChatModelApi {
   ...
   * io.camunda.connector.agenticai.aiagent.capabilities.ModelCapabilitiesResolver#matches}), as
   * opposed to falling through to family/conservative defaults. Threaded into the request converter
   * so {@link AnthropicReasoningValidator} can distinguish "declared but not reasoning-capable"
   * (validate) from "unknown/custom model" (pass-through).
   */
