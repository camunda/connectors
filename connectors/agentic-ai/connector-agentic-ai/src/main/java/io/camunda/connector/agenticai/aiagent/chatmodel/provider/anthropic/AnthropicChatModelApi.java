...
/**
 * Anthropic {@link ChatModel}: drives the vendor SDK's streaming beta Messages endpoint for every
 * call (Anthropic has no meaningful non-streaming distinction for this connector's purposes;
 * streaming is used uniformly to accumulate the same {@link BetaMessage} shape the non-streaming
 * API would return), then delegates to the Task 3/4/5 converters to translate to/from the domain
 * model.
 *
