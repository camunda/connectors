...
class AgenticAiConnectorsAutoConfigurationTest {
  ...
  private static final List<Class<?>> AGENT_BEANS =
      List.of(
  ...
  private static final List<Class<?>> SCHEMA_BEANS =
      List.of(
  ...
  private static final List<Class<?>> ALL_AGENT_BEANS =
      Stream.concat(AGENT_BEANS.stream(), LANGCHAIN4J_BEANS.stream()).toList();

  private static final List<Class<?>> ALL_BEANS =
      Stream.of(AGENT_BEANS.stream(), SCHEMA_BEANS.stream(), LANGCHAIN4J_BEANS.stream())
          .flatMap(s -> s)
          .toList();
  ...
  private final ApplicationContextRunner contextRunner =
      new ApplicationContextRunner()
          ...
          .withUserConfiguration(AgenticAiSchemaAutoConfiguration.class);
