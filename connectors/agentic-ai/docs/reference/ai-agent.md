...
Definition: [`AgentContext.java`](../../src/main/java/io/camunda/connector/agenticai/aiagent/model/AgentContext.java).
Its components:

- `state`: lifecycle state (`INITIALIZING`, `TOOL_DISCOVERY`, `READY`)
- `metadata`: process definition / instance keys (used for migration detection)
- `metrics`: cumulative model calls and token usage
- `toolDefinitions`: the resolved tools available to the LLM
- `conversation`: the memory storage reference (the `ConversationContext` pointer)
- `properties`: extensible map (e.g. gateway tool discovery state)
...
Definition: [`AgentResponse.java`](../../src/main/java/io/camunda/connector/agenticai/aiagent/model/AgentResponse.java).
Its components:

- `context`: the updated `AgentContext`
- `toolCalls`: the tool calls to execute (as `ToolCallProcessVariable`)
- `responseMessage`: the full assistant message (optional)
- `responseText`: the text response (optional)
- `responseJson`: the parsed JSON response (optional)

### AiAgentSubProcessConnectorResponse (job worker specific)

Implements `AdHocSubProcessConnectorResponse` with job completion control. Definition:
...
- `agentResponse`: the wrapped `AgentResponse`
- `completionConditionFulfilled`: `true` completes the AHSP
- `cancelRemainingInstances`: `true` cancels still-running tool instances
