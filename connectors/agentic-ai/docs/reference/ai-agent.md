...
**Job leasing (v2 connectors)**: The v2 AI Agent connectors (`AgentTaskV2Function`, `AgentSubProcessV2Function`) opt into job leasing via `@OutboundConnector(withLease = true)`. Each activation carries a per-activation `leaseToken` (`JobContext#getLeaseToken()`), and the runtime completes/fails jobs through the `ActivatedJob`-based command overloads, so completion is fenced against a superseded activation automatically. The agent-instance writes are fenced too — `CamundaAgentInstanceClient` forwards the token via `jobLease(...)` on the update and create-history commands, so a superseded activation's writes are discarded server-side (see [§23](#23-agent-instance-integration)). Note the version-skew contract on `OutboundConnector#withLease`: over gRPC a pre-leasing gateway drops the field (token is `null`), so the client only forwards a lease when one is present; over REST an older gateway rejects the activation (HTTP 400).
...
**Job-lease fencing.** With the v2 connectors leased ([§10](#10-concurrency-challenges--race-conditions)), the activation's `leaseToken` reaches `CamundaAgentInstanceClient`, which forwards it via `jobLease(...)` on both write commands (`newUpdateAgentInstanceCommand`, `newCreateAgentHistoryItemCommand`) whenever a token is present. The engine correlates a write to the active job by `jobKey` and rejects it (`NOT_FOUND`, "supplied lease does not match") if the supplied lease doesn't match — so a superseded activation's status/metrics/history writes are discarded rather than committed. A token is absent (and no lease forwarded) for v1 connectors and for the gRPC version-skew case; those writes proceed unfenced as before.
