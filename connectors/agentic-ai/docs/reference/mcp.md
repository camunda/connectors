...
├── McpClientErrorCodes                         # error code constants
├── client/
│   ├── McpClientFunction                       # outbound connector (runtime-configured)
│   ├── McpRemoteClientFunction                 # outbound connector (remote)
│   ├── McpClientRegistry                       # runtime client lifecycle (lazy init)
│   ├── McpRemoteClientRegistry                 # remote client cache (Caffeine)
│   ├── McpClientResultDocumentHandler          # binary → Camunda document conversion
│   ├── configuration/
│   │   ├── annotation/
│   │   │   ├── @RuntimeMcpClientFactory        # Spring @Qualifier for runtime factory
│   │   │   └── @RemoteMcpClientFactory         # Spring @Qualifier for remote factory
│   │   ├── McpClientBaseConfiguration          # shared executor + headers beans
│   │   ├── McpClientConfiguration              # runtime client Spring config (opt-in)
│   │   ├── McpClientConfigurationProperties    # runtime client Spring Boot properties
│   │   ├── McpRemoteClientConfiguration        # remote client Spring config (on by default)
│   │   ├── McpRemoteClientConfigurationProperties # remote client cache properties
│   │   ├── McpDocumentHandlerConfiguration     # document handler bean
│   │   ├── validation/
│   │   │   ├── @ValidMcpClientConfiguration    # custom constraint annotation
│   │   │   └── McpClientConfigurationValidator # JSR-380 validator
│   │   └── mcpsdk/
│   │       ├── McpSdkMcpClientConfiguration    # conditional runtime SDK config
│   │       └── McpSdkMcpRemoteClientConfiguration # conditional remote SDK config
│   ├── execution/
│   │   ├── McpClientDelegate                   # per-operation MCP interface
│   │   └── McpClientExecutor                   # dispatches operation → delegate method
│   ├── filters/
│   │   ├── AllowDenyList                       # allow/deny filter by name
│   │   └── FilterOptions                       # holder for tools/resources/prompts filters
│   ├── framework/
│   │   ├── bootstrap/
│   │   │   ├── McpClientHeadersSupplierFactory # builds composite header supplier
│   │   │   └── auth/
│   │   │       ├── BasicAuthHeadersSupplier
│   │   │       ├── BearerAuthHeadersSupplier
│   │   │       └── OAuthHeadersSupplier
│   │   └── mcpsdk/
│   │       ├── McpSdkClientFactory             # creates MCP SDK client instances
│   │       └── rpc/                            # package-private request classes
│   │           ├── McpSdkMcpClientDelegate     # McpClientDelegate via McpSyncClient
│   │           ├── ListToolsRequest            # tools/list
│   │           ├── ToolCallRequest             # tools/call
...
│   ├── handler/
│   │   ├── McpClientHandler                    # interface: handle(context, McpClientRequest)
│   │   ├── McpRemoteClientHandler              # interface: handle(context, McpRemoteClientRequest)
│   │   ├── DefaultMcpClientHandler             # runtime impl: registry lookup
│   │   └── DefaultMcpRemoteClientHandler       # remote impl: cache get/create
│   └── model/
│       ├── auth/
│       │   ├── Authentication                  # sealed: None|Basic|Bearer|OAuth
│       │   └── ...                             # concrete subtypes
│       ├── content/
│       │   ├── McpContent                      # sealed: all content variants
│       │   ├── McpTextContent, McpBlobContent, McpObjectContent
│       │   ├── McpEmbeddedResourceContent      # Text|Blob|BlobDocument variants
│       │   ├── McpResourceLinkContent
│       │   └── McpDocumentContent              # Camunda Document reference
│       ├── result/
│       │   ├── McpClientResult                 # sealed: all result types
│       │   ├── McpClientResultWithStorableData # sealed sub: results with binaries
│       │   ├── McpClientListToolsResult, McpClientCallToolResult
│       │   ├── McpClientListResourcesResult, McpClientReadResourceResult
│       │   ├── McpClientListPromptsResult, McpClientGetPromptResult
│       │   └── ...                             # supporting records
│       ├── McpClientOperation                  # sealed: method + params
│       ├── McpClientOperationDefinitions       # factory: listTools(), callTool()
│       ├── McpConnectorModeConfiguration       # sealed: ToolMode | StandaloneMode
│       ├── McpClientRequest                    # connector input (runtime)
│       ├── McpRemoteClientRequest              # connector input (remote)
│       ├── McpRemoteClientTransportConfiguration # sealed: StreamableHttp | SSE
│       ├── McpToolDefinition                   # name, title, description, inputSchema
│       └── ...                                 # filter configs
└── discovery/
    ├── McpToolCallIdentifier                   # naming: MCP_<element>___<tool>
    ├── McpClientGatewayToolHandler             # GatewayToolHandler impl for MCP
    ├── McpClientGatewayToolDefinitionResolver  # detects MCP elements in BPMN
    └── configuration/
        └── McpDiscoveryConfiguration           # Spring config for discovery beans
```
