...
public class GoogleDriveService {
    ...
    public GoogleDriveResult execute(final GoogleDriveClient client, final Resource resource) {
        return switch (resource.type()) {
            ...
            case UPLOAD -> uploadFile(client, resource.uploadData().document());
            case DOWNLOAD -> null;
        };
    }
    ...
    private List<Request> mapJsonToRequests(String requestsAsJson) {
        try {
            ...
            java.lang.reflect.Type listType = new TypeToken<ArrayList<Request>>() {
            }.getType();
    ...
    private GoogleDriveResult uploadFile(final GoogleDriveClient client, final Document document){
        Drive drive = client.getDriveService();

        var documentReference = (DocumentReference.CamundaDocumentReference) document.reference();
        File fileMeta = new File();
        fileMeta.setId(documentReference.documentId());
        fileMeta.setName(document.metadata().getFileName());


        //  FileContent fileContent = new FileContent("")

        var content = new ByteArrayContent("image/png", document.asByteArray());
