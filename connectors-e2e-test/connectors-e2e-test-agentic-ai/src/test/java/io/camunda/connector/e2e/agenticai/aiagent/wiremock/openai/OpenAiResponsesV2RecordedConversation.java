...
public final class OpenAiResponsesV2RecordedConversation {
  ...
  public static final class RecordedChatRequest {
    ...
    /**
     * A {@code function_call_output} item's {@code output} field is a union: either a plain string,
     * or (what {@code OpenAiResponsesRequestConverter} always sends - see {@code
     * toolResultInputItems}) an array of output-content items (e.g. {@code
     * [{"type":"input_text","text":"24"}]}). {@link JsonNode#asText()} on an array node silently
     * returns {@code ""} rather than the text it carries, so the array shape must be unwrapped
     * explicitly - joining every {@code input_text} item's {@code text} field; other item kinds
     * (e.g. {@code input_image}/{@code input_file} tool-result attachments) have no plain-text
     * representation and are skipped, same as {@link #parseContent}'s treatment of non-text content
     * parts.
     */
    private static String outputText(JsonNode outputNode) {
      ...
      outputNode.forEach(
          item -> {
            if ("input_text".equals(item.path("type").asText())) {
     ...
     * #messages()} (which regroups items and silently skips kinds with no provider-neutral
     * representation), this exposes every item verbatim and in original order.
     */
