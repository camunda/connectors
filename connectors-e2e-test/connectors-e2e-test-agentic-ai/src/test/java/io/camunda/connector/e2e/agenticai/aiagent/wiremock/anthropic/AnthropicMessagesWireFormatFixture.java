 ...
 *
 * <p>Notable wire-level differences from OpenAI's Chat Completions format, pinned down here as
 * regression coverage ahead of the native-provider rewrite (#7211):
 *
 * <ul>
 ...
 *   <li>JSON-schema structured output is a genuinely native wire field ({@code
 *       output_config.format}), unlike providers without one — but the schema <em>name</em>
 *       configured on the connector is dropped; only the raw JSON schema is sent.
...
public final class AnthropicMessagesWireFormatFixture implements ProviderWireFormatFixture {
  ...
  public void assertResponseFormatConfigured(
    ...
    assertThat(responseFormat).as("output_config.format in recorded request").isPresent();
    assertThat(responseFormat.get().type()).isEqualTo("json_schema");
    // Note: Anthropic's wire format has no field for the schema *name* configured on the
    // connector (data.response.format.schemaName) — only the raw JSON schema is sent, so
    // expectedSchemaName is intentionally not asserted here.
