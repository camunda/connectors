...
/**
 * Preliminary wire-format e2e coverage per LLM provider/API, extending the WireMock-based approach
 * introduced in #7400 (until now only covering {@code openaiCompatible}) to the other providers
 * supported by the AI Agent connector.
 *
 * <p>This is a <b>regression safety net</b> for the "Own the LLM layer" native-provider rewrite
 * (#7211): it targets {@code main}'s current LangChain4j-mediated wire behavior as a baseline, so
 * that any wire-format drift introduced by the native rewrite becomes visible instead of silent. It
 * deliberately does not duplicate the full behavioral e2e suite that already exists for {@code
 * openaiCompatible} (tool calling, feedback loops, memory storage, MCP/A2A, etc.) — one row per
 * provider/API supplies a {@link ProviderWireFormatFixture}, and the four scenarios below cover the
 * wire-format areas most at risk of drifting: plain text, tool calls, multimodal user-prompt
 * documents, and structured JSON output.
 *
...
public class ProviderWireFormatSmokeTests extends BaseAiAgentJobWorkerTest {
  ...
  void jsonResponseSchemaStructuredOutput() throws Exception {
    ...
    assertThat(fixture.modelCallCount()).isEqualTo(1);
    fixture.assertResponseFormatConfigured(fixture.lastRecordedRequest(), schemaName);
