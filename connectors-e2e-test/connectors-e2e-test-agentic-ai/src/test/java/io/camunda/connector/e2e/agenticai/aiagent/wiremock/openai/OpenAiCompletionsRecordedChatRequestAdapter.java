...
/**
 * Adapts {@link OpenAiCompletionsRecordedConversation.RecordedChatRequest} (and its nested
 * message/tool-call types) to the provider-agnostic {@link RecordedChatRequest} SPI, without
 * touching the original class which the existing OpenAI-compatible e2e test suite depends on
 * directly.
 */
final class OpenAiCompletionsRecordedChatRequestAdapter implements RecordedChatRequest {

  private final OpenAiCompletionsRecordedConversation.RecordedChatRequest delegate;

  OpenAiCompletionsRecordedChatRequestAdapter(
      OpenAiCompletionsRecordedConversation.RecordedChatRequest delegate) {
  ...
  private record ToolCallAdapter(
      OpenAiCompletionsRecordedConversation.RecordedMessage.RecordedToolCall delegate)
