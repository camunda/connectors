...
public class L4JAiAgentConnectorHttpTimeoutTests extends BaseAiAgentConnectorTest {
  ...
  private static final Duration MODEL_TIMEOUT = Duration.ofSeconds(4);
  ...
  @Test
  void anthropic_processCompletesWhenResponseArrivesWithinSocketTimeout() throws Exception {
    stubAnthropic(RESPONSE_DELAY_BELOW_TIMEOUT);
    runPositiveCase(anthropicProvider());
  }

  @Test
  void anthropic_raisesIncidentWhenResponseExceedsSocketTimeout() throws Exception {
    stubAnthropic(RESPONSE_DELAY_ABOVE_TIMEOUT);
    runNegativeCase(anthropicProvider());
  }

  // ---------------------------------------------------------------------------
  // OpenAI-compatible (covers the JDK HTTP client path with a configurable endpoint)
  // ---------------------------------------------------------------------------

  @Test
  void openAiCompatible_processCompletesWhenResponseArrivesWithinSocketTimeout() throws Exception {
    stubOpenAiCompatible(RESPONSE_DELAY_BELOW_TIMEOUT);
    runPositiveCase(openAiCompatibleProvider());
  }

  @Test
  void openAiCompatible_raisesIncidentWhenResponseExceedsSocketTimeout() throws Exception {
    stubOpenAiCompatible(RESPONSE_DELAY_ABOVE_TIMEOUT);
    runNegativeCase(openAiCompatibleProvider());
  }

  // ---------------------------------------------------------------------------
  // Bedrock (Apache HTTP client path — the original #7193 culprit)
  // ---------------------------------------------------------------------------

  @Test
  void bedrock_processCompletesWhenResponseArrivesWithinSocketTimeout() throws Exception {
    stubBedrock(RESPONSE_DELAY_BELOW_TIMEOUT);
    runPositiveCase(bedrockProvider());
  }

  @Test
  void bedrock_raisesIncidentWhenResponseExceedsSocketTimeout() throws Exception {
  ...
  private Function<ElementTemplate, ElementTemplate> anthropicProvider() {
    return template ->
        template
            ...
            .property("provider.anthropic.endpoint", wm.getHttpBaseUrl())
