...
abstract class BaseMistralSubProcessTest extends BaseAgentSubProcessTest {
  ...
  protected Function<ElementTemplate, ElementTemplate> providerConfigurer() {
    return this::configureMistralBackend;
  }
  ...
  private ElementTemplate configureMistralBackend(ElementTemplate template) {
    return template
        .property("provider.type", "mistral")
        .property("provider.mistral.backend.type", "mistral-api")
        .property("provider.mistral.backend.mistral.endpoint", wireMock.getHttpBaseUrl() + "/v1")
        .property("provider.mistral.backend.mistral.apiKey", "dummy")
        .property("provider.mistral.model.model", defaultModel());
  }
  ...
  JsonNode parseBody(LoggedRequest loggedRequest) {
    try {
      ...
      throw new IllegalStateException(
          "Failed to parse recorded Mistral Chat Completions request body: "
