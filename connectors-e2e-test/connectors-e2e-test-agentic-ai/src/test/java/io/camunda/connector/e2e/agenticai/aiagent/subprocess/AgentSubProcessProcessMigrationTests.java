...
import io.camunda.connector.e2e.agenticai.aiagent.AgentToolSpecifications;
...
@SlowTest
public class AgentSubProcessProcessMigrationTests extends BaseAgentSubProcessTest {
  ...
  void updatesToolDefinitionsAfterMigration() throws IOException {
    ...
    final var getDateAndTimeTool =
        recorded.lastRequest().toolDefinitions().stream()
            .filter(t -> t.name().equals(AgentToolSpecifications.GET_DATE_AND_TIME_TOOL_NAME))
            .findFirst()
            .orElseThrow();
