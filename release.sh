...
mvn --batch-mode versions:set -DnewVersion=${RELEASE_VERSION} -DgenerateBackupPoms=false -f parent
mvn --batch-mode package
git commit -a -m "${RELEASE_MESSAGE}"
git tag -a ${RELEASE_VERSION} -m "${RELEASE_MESSAGE}"
mvn --batch-mode versions:set -DnewVersion=${NEXT_VERSION} -DgenerateBackupPoms=false -f parent
