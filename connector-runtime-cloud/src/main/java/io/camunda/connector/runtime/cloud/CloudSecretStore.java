...
public class CloudSecretStore implements SecretStore {

  private static final Logger LOGGER = LoggerFactory.getLogger(CloudSecretStore.class);
  ...
  public CloudSecretStore(final Gson gson, final String clusterId) {
    final String json =
        Optional.ofNullable(clusterId)
            .map(CloudSecretStore::loadGoogleSecrets)
            .or(CloudSecretStore::loadEnvironmentSecrets)
            .or(CloudSecretStore::loadPropertiesSecrets)
            .orElse("{}");
