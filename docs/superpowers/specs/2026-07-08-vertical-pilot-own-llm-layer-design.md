...
- **Consume the vendor APIs in a streamable fashion (required).** Each native implementation drives the provider's **streaming** endpoint internally and assembles the full `ChatModelResult` (robust against server-side timeouts on long generations; the BPMN job-worker model surfaces only the assembled result, not live tokens). This keeps the door open to retrofit an optional SPI-level streaming/observability surface — a `ChatStreamListener` + discriminated `ChatModelEvent` taxonomy (and possibly a `CompletableFuture` return), per the #7151 hackdays design — without re-touching any provider. **That SPI-level surface is an OPEN decision (leaning adopt — cheap now, expensive after the natives land); do it as a dedicated pre-C7 SPI chunk if adopted. Our `Completed | Continuation` result stays either way.** The `CompletableFuture` half buys nothing until the handler chain is async (out of scope), so it is the weaker half.
