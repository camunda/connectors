...
- **Runtime conversion.** `DocumentReturnProcessor.process(DocumentReturn<?>, DocumentReturnFormat)` reads the user's choice and encoding, streams the `RawPayload` through a try-with-resources so no buffering is required for `DOCUMENT`, enforces the 1.5 MiB inline-payload guard (`InlineSizeGuard.MAX_INLINE_BYTES`) for `TEXT`/`JSON` (fix for #7049), and invokes `wrap` with the converted value. For `TEXT`/`JSON` the runtime reads at most `MAX_INLINE_BYTES + 1` bytes before applying the guard, so an oversized remote object fails with a controlled incident instead of exhausting the runtime heap. This runs before result-expression evaluation, so downstream expressions see the converted value in the shape the connector defines. A safety-net catch in `SpringConnectorJobHandler` closes the payload stream if `process()` throws before it acquires ownership.
...
- **Generator annotation.** `@DocumentReturnFormat` is a **type-level** annotation (`@Target(TYPE)`), placed on the request **download sub-type** — S3 `DownloadObject`, GCS `DownloadObject`, Azure `DownloadBlob`, Drive `DownloadData`, and Box's nested download type. It auto-generates the dropdown, its condition guard (visibility gated on the download-operation discriminator), and the encoding sub-field. Regardless of where the annotation sits, the dropdown always binds to the root-level `documentReturnFormat.choice` / `.encoding` path so the runtime can read it back uniformly. For S3, GCS and Azure the sub-type is not a Jackson path segment (they use `@NestedProperties(addNestedPath = false)`), so no path prefix is applied; for Box and Drive the `DiscriminatorPropertyBuilder` rewrites the condition when sealed-subtype nesting path-prefixes the dropdown id.
