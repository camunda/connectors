...
1. **Function shape.** `createDocument(value)`:
   ...
   - The function returns a sentinel `ValContext` tagged with a **new** discriminator (distinct from `IntrinsicFunctionModel.DISCRIMINATOR_KEY`, which remains reserved for input-side functions and stays forbidden in output), e.g. `connectorResultFunction: "createDocument:<nonce>"`. The nonce is a random UUID scoped to the *current result/error expression evaluation* (`FeelConnectorFunctionProvider#beginCreateDocumentEvaluationScope`/`currentCreateDocumentNonce`, backed by a `ThreadLocal`), generated lazily on first use within that scope — not a single per-JVM constant. A per-JVM constant was the initial design, but it's a plain FEEL-visible string once produced: nothing stops an expression from projecting it back out (e.g. `createDocument(x).connectorResultFunction`), so any tenant able to author a result/error expression on a shared runtime could learn it and later forge sentinels in another tenant's response/request data. Scoping a fresh nonce per evaluation means a value learned from one evaluation is worthless against any other — `ResultDocumentResolver.resolve`/`isCreateDocumentSentinel` only recognize a sentinel tagged with the exact nonce of the evaluation currently resolving it.
...
- **No breaking changes.** Additive function; existing toggles, existing `documents[]` webhook behavior, and every pre-existing public constructor on `ConnectorResultHandler`/`InboundCorrelationHandler`/`MeteredInboundCorrelationHandler` continue to work (a delegating legacy overload was kept on each, since the `DocumentFactory` parameter added to reach `createDocument()` would otherwise have been a binary/source break for runtime extensions built against the previous signatures).
- **`DocumentFactory` work happens at the walker step**, which already has natural per-job scope, rather than threading it into the FEEL engine itself.

### Negative

- **Sentinel walking adds a tree-traversal step to result/error-expression evaluation** when a `createDocument` call is present. Guarded by a cheap `String.contains` check on the discriminator property beforehand, so evaluations that never call `createDocument` skip the parse/walk/reserialize (and the nonce's `UUID.randomUUID()` generation) entirely and see zero added cost.
- **The nonce requires a `ThreadLocal` scope around every result/error-expression evaluation** (`ConnectorResultHandler.createOutputVariables`/`examineErrorExpression`), opened before `evaluateToJson` and closed in a `finally` block — the one piece of mutable, thread-scoped state in an otherwise stateless FEEL function SPI. This is a deliberate, narrow exception to keep the anti-forgery property real (see Decision §1): a static per-JVM secret is observable and reusable across tenants once any one expression projects it out, so the nonce must be scoped no wider than a single evaluation.
...
- **`InboundCorrelationHandler`'s constructor signature changes**, along with its Spring wiring chain (`InboundCorrelationConfiguration`, `MeteredInboundCorrelationHandler`) — every call site and test double for these classes needs updating (mitigated by the legacy overloads noted above).
