...
1. **Function shape.** `createDocument(value)`:
   ...
   - This covers both forms from the architecture session — `createDocument(response.body.file[1])` (an object with recognized keys) and `createDocument(response.body.file[1].document.data)` (a bare string) — without requiring the caller to reshape data into a fixed envelope.
   - When `name`/`fileName` is omitted, a random UUID is used as the filename; when `contentType` is omitted, it's inferred from the decoded bytes, falling back to `application/octet-stream` if inference fails. This mirrors the existing default behavior for inline documents established in ADR-0005 Step 1, rather than introducing a third convention.
   - The function returns a sentinel `ValContext` tagged with a **new** discriminator (distinct from `IntrinsicFunctionModel.DISCRIMINATOR_KEY`, which remains reserved for input-side functions and stays forbidden in output), e.g. `connectorResultFunction: "createDocument"`.
...
- **Sentinel walking adds a tree-traversal step to every result/error-expression evaluation**, even when no `createDocument` call is present (mitigated: the walk is a cheap no-op scan when the discriminator key is absent).
