...
### LocalStack's licensing change (checked against LocalStack's own site and Docker Hub's API, 2026-07-28)

- LocalStack ended support for the LocalStack for AWS Community edition on **March 23, 2026**. From that date, LocalStack ships a single image on Docker Hub that requires a user account and an auth token **to start** — the gate applies when the container runs, not when the image is pulled; the image itself stays publicly downloadable.
- A temporary bypass (`LOCALSTACK_ACKNOWLEDGE_ACCOUNT_REQUIREMENT=1`) was available but expired **April 6, 2026**.
- Our pinned `4.8` tag predates this change. Checked against Docker Hub's API today: `localstack/localstack:4.8` has `tag_status: active`, was last pushed 2025-09-16, and was last pulled 2026-07-28 — it is still published and pullable. Whether it also still starts without an account/token was not re-verified in this session (no container was run); the CI lane's own green runs are the evidence for that. Any *upgrade* past `4.8` would pull the new single image, which does require an account and token to start.
...
No code change is needed to adopt a mirror later. Testcontainers 2.0.5 (the version pinned in `parent/pom.xml`) already substitutes image names via the `TESTCONTAINERS_HUB_IMAGE_NAME_PREFIX` environment variable (or the `hub.image.name.prefix` config property), applied unconditionally through `DefaultImageNameSubstitutor`. This was confirmed by inspecting `testcontainers-2.0.5.jar`'s bytecode directly (not just the public docs, since the jar is what's actually pinned): `PrefixingImageNameSubstitutor` reads `hub.image.name.prefix`, which `TestcontainersConfiguration`'s key-to-env-var mapping turns into `TESTCONTAINERS_HUB_IMAGE_NAME_PREFIX`; it no-ops when the prefix is unset; it skips any image whose `DockerImageName` already has a non-empty registry (logging "not a Docker Hub image"), which is why `quay.io/keycloak/keycloak:26.5` and `registry.camunda.cloud/...` references are left untouched; and `ImageNameSubstitutor`'s static initializer wires this in by default, with no SPI registration needed. The substitution runs against the resolved `DockerImageName` at pull time — after a container's constructor has already asserted compatibility against the *unprefixed* name supplied in code — which is structurally why this works for `LocalStackContainer`/`KafkaContainer` and similar specialized containers: prefixing the string earlier, e.g. inside `DockerImages.get()` before it reaches the constructor, would fail that compatibility check instead. A trailing slash on the prefix value is required (`registry.camunda.cloud/mirror/`, not `registry.camunda.cloud/mirror`): the substitutor concatenates the prefix's parsed repository segment directly onto the original repository string, with no separator inserted.
