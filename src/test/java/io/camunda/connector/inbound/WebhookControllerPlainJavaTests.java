...
public class WebhookControllerPlainJavaTests {
    ...
    public void webhookMultipleVersions() throws IOException {
        ...
        Collection<WebhookConnectorProperties> connectors1 = registry.getWebhookConnectorByContextPath("myPath");
        assertEquals(1, connectors1.size()); // only one
        assertEquals(2, connectors1.iterator().next().getVersion()); // And the newest one

        Collection<WebhookConnectorProperties> connectors2 = registry.getWebhookConnectorByContextPath("myPath2");
    ...
    public static void register(InboundConnectorRegistry registry, String bpmnProcessId, int version, String contextPath) {
        register(registry, ++nextProcessDefinitionKey, bpmnProcessId, version, contextPath);
    }

    public static void register(InboundConnectorRegistry registry, long processDefinitionKey, String bpmnProcessId, int version, String contextPath) {
        registry.markProcessDefinitionChecked(processDefinitionKey, bpmnProcessId, version);
        if (contextPath!=null) {
            registry.registerWebhookConnector(webhookProperties(processDefinitionKey, bpmnProcessId, version, contextPath));
        }
    }
    ...
    public static InboundConnectorProperties webhookProperties(long processDefinitionKey, String bpmnProcessId, int version, String contextPath) {
        return new InboundConnectorProperties(bpmnProcessId, version, processDefinitionKey, Map.of(
