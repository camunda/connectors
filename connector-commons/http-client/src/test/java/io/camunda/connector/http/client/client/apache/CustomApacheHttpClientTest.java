...
//              .withClasspathResourceMapping("passwords", "/etc/squid/passwords",
// BindMode.READ_ONLY)
...
//      proxy.stubFor(
//          post("/path").willReturn(created().withJsonBody(new POJONode(Map.of("key1",
// "value1")))));
//      setAllSystemProperties();
//
//      HttpClientRequest request = new HttpClientRequest();
...
//        WireMockRuntimeInfo wmRuntimeInfo) {
//      stubFor(get("/path").willReturn(ok().withHeader(headerKey, "Test-Value-1",
// "Test-Value-2")));
//      HttpClientRequest request = new HttpClientRequest();
//      request.setMethod(HttpMethod.GET);
//      request.setUrl(wmRuntimeInfo.getHttpBaseUrl() + "/path");
...
//      assertThat(result).isNotNull();
//      assertThat(result.status()).isEqualTo(200);
//      assertThat(result.headers().get(headerKey) instanceof
// List).isEqualTo(expectedDoesReturnList);
...
//          objectMapper.readValue(objectMapper.writeValueAsString(request),
// HttpClientRequest.class);
//      assertEquals("\"Hello, world\"", parsedRequest.getBody());
//      HttpClientRequest parsedResult =
//          objectMapper.readValue(objectMapper.writeValueAsString(result),
// HttpClientRequest.class);
...
//          .contains("An error occurred while executing the request, or the connection was
// aborted");
//    }
//  }
//
...
// boundary=g7wNbtOKHnEq4vnSoWdDYS88OICfGHzBA68DqmJS"));
//      request.setBody(Map.of("key1", "value1", "key2", "value2"));
//      request.setUrl(wmRuntimeInfo.getHttpBaseUrl() + "/path");
//      HttpClientResult result = customApacheHttpClient.execute(request);
//      assertThat(result).isNotNull();
...
//              "read:resource"));
//      request.setUrl(wmRuntimeInfo.getHttpBaseUrl() + "/path");
//      var e = assertThrows(ConnectorException.class, () ->
// customApacheHttpClient.execute(request));
