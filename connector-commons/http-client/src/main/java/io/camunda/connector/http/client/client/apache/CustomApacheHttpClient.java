...
public class CustomApacheHttpClient implements HttpClient {
  ...
  /**
   * Executes the request and returns a snapshot of the response with the body stream still live.
   * Closing {@link StreamingHttpResponse#body()} releases the response, returns the connection to
   * the pool, and disposes the underlying HTTP client.
   *
   * <p>Use this when the body is handed to a downstream consumer that owns the stream's lifetime
   * (e.g. the runtime's {@code DocumentReturnProcessor} consuming a {@link
   * io.camunda.connector.api.document.DocumentReturn}'s payload). The caller MUST close the body
   * stream exactly once — otherwise the client and connection leak.
   */
  @Override
  ...
  /**
   * Wraps the response entity's input stream so closing it cascades — entity stream → response →
   * client — releasing the connection and disposing the client. Tolerates a {@code null} delegate
   * stream (empty body) and propagates only the first close exception.
   */
