 ...
 * first {@link #isAllowed(Call)} call rather than at construction time — mirroring {@link
 * LazyLoadingSecretFilter}. {@code IntrinsicFunctionUtil}'s bound-tree walk only ever calls {@code
 * isAllowed} when a job's variables actually contain a {@code camunda.function.type} discriminator
 * somewhere, so building the allow-list eagerly at {@link
 * ConfigurableIntrinsicFunctionAllowListFactory#create} time — as this class's predecessor did —
 * fetched and parsed the deployed BPMN model for every single job, including the overwhelming
 * majority that never use an intrinsic function at all, rather than only the jobs that actually
 * need the check. Deferring the fetch here also means a fetch/parse failure surfaces from {@code
 * isAllowed} — reached from within the connector's own guarded execution — the same place a secret
 * lookup failure already does, rather than from {@code create()}, called before that guard.
 *
