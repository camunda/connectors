...
public interface IntrinsicFunctionAllowListFactory {
   ...
   * every call rather than permitting them — unconditional intrinsic-function dispatch is
   * security-testing-findings#275's actual RCE-class primitive, so this can never become a silent
   * fail-open if a future caller reaches for it expecting a safe, no-op default.
   */
