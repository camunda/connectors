...
public class OutboundConnectorsAutoConfiguration {
   ...
   * ...) all bind data that arrived as external payload, not a model author's FEEL text -- the same
   * "cannot tell trusted model text from payload data" situation {@link
   * DisabledIntrinsicFunctionExecutor} documents. None of them run an {@link
