...
/**
 * Request-level test for the FormContentFilter gap flagged in review on PR #9012: Spring Boot's
 * default {@code FormContentFilter} parses {@code application/x-www-form-urlencoded} bodies for
 * PUT/DELETE requests in a servlet filter that runs before {@code DispatcherServlet} — before path
 * resolution, rate limiting, or {@code InboundWebhookRestController}'s own body-size guard. Like
 * {@link WebhookMultipartSizeLimitTest}, this needs a real embedded server: none of this repo's
 * other webhook tests dispatch through the actual filter chain.
 */
@SpringBootTest(
...
class WebhookFormContentFilterExclusionTest {
  ...
  void shouldStillProcessWithinLimitFormUrlEncodedPutToRegisteredPath() throws Exception {
    // correlationHandler here is the real, Spring-managed MeteredInboundCorrelationHandler (as in
    // WebhookControllerTestExceptionZeebeTest), not a mock -- the deterministic failure comes from
    // camundaClient instead, exactly like that sibling test class.
    ...
    // "a=1" is 3 bytes, within the 8-byte max-request-body-bytes configured above.
    var response = sendFormRequest("PUT", "/inbound/formPath", "a=1");

    // Reaches correlation, which fails with the stubbed gRPC status mapped to 400 -- proving the
    // filter exclusion didn't break ordinary webhook processing for a within-limit
    // form-urlencoded PUT (it isn't 404, 413, or 429).
