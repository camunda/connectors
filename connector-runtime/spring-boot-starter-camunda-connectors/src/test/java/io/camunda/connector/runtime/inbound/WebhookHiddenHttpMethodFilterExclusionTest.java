...
/**
 * Request-level test for the {@code HiddenHttpMethodFilter} gap flagged in review on PR #9012: when
 * {@code spring.mvc.hiddenmethod.filter.enabled=true} (off by default), Spring Boot's
 * auto-configured filter reads a form-urlencoded POST body looking for a {@code _method} parameter,
 * in a servlet filter that runs before {@code DispatcherServlet} -- before path resolution, rate
 * limiting, or {@code InboundWebhookRestController}'s own body-size guard. Like {@link
 * WebhookFormContentFilterExclusionTest}, this needs a real embedded server.
 */
@SpringBootTest(
...
class WebhookHiddenHttpMethodFilterExclusionTest {
  ...
  @Test
  void shouldReturn413ForOversizedFormUrlEncodedPostToRegisteredPath() throws Exception {
    ...
    // 64 bytes: over the 8-byte max-request-body-bytes configured above. If the filter exclusion
    // regressed, HiddenHttpMethodFilter would consume this itself (no size limit) looking for
    // "_method", leaving readBoundedBody nothing to reject.
    var response = sendFormRequest("POST", "/inbound/formPath", "field=" + "x".repeat(64));
